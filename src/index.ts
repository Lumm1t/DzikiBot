...
async function msgProcessor(msg: Discord.Message) {
  ...
  let subject = msg.mentions.members?.first();
  ...
  try {
    ...
    let channelID = await database.getLogChannel(msg.guild!.id);
    ...
    let publicLogChannel = channel as Discord.TextChannel;
    switch (args[0]) {
      case "kick":
        if (!msgAuthor.hasPermission("KICK_MEMBERS") || subject?.hasPermission("ADMINISTRATOR")) {
        ...
        let kickMessage = await messages.kickServerMessage(msg, args, subject, delivered);
        publicLogChannel.send(kickMessage)
        subject.kick();
        background.waitAndDelete(msg, 10000);
        break;
      case "ban":
        if (!msgAuthor.hasPermission("BAN_MEMBERS") || subject?.hasPermission("ADMINISTRATOR")) {
          throw EndingMessage.NoPermissionOrUserIsAdmin;
        }
        if (!subject) {
          throw EndingMessage.IncorrectUserData;
        ...
        let banMessage = await messages.banServerMessage(msg, args, subject, delivered);
      ...
      case "mute": {
        ...
        if (args[2] == null) {
          let muteMessage = await messages.permMuteMessage(msg, subject);
          ...
          subject.roles.add(role);
          let muteMessage = await messages.muteMessage(msg, args, subject);
          publicLogChannel.send(muteMessage);
          background.waitAndDelete(msg, 10000);
        }
        break;
      ...
      case "unmute": {
        ...
        publicLogChannel.send(`<@${subject!.id}> został odmutowany przez: <@${msg.member!.id}>!`);
      ...
      case "tempban":
        ...
        let tempBanMessage = await messages.tempBanMessage(msg, args, subject, delivered);
      ...
      case "warn": {
        ...
        let time = await modules.setEndingDate("3d", "db", false);
        let warnCount = await database.warn(subject, time);
        if (warnCount != 3) {
          let warnMessage = await messages.warnMessage(msg, args, subject, warnCount, "0");
          publicLogChannel.send(warnMessage);
          background.waitAndDelete(msg, 10000);
        } else {
          let muteTime = await database.warnTime(msg);
          ...
          let warnMessage = await messages.warnMessage(msg, args, subject, warnCount, date);
          publicLogChannel.send(warnMessage);
        }
        break;
      }
      case "mutetime":
        if (msgAuthor.hasPermission("ADMINISTRATOR")) {
          ...
          background.drd(msg, "Czas muta po 3 warnach został pomyślnie ustawiony na: " + args[1]);
        }
        break;
      case "prywatnelogi":
        if (msgAuthor.hasPermission("ADMINISTRATOR")) {
          ...
          if (channel) {
            if (channel.type == "text") {
              ...
              background.drd(msg, "Udane ustawienie prywatnych logów na kanał: " + channel.name);
            }
          } else {
            background.drd(msg, "Musisz oznaczyć kanał tekstowy. $prywatnelogi #kanał");
          }
        }
        break;
      case "statystyki":
      ...
      case "blacklist":
        ...
        let channelsList = await database.getBlacklist(msg.guild!);
        switch (args[1]) {
          case "list": {
            if (channelsList) {
              ...
              channels.forEach(channelID => {
                let channel = msg.guild!.channels.cache.get(channelID) as Discord.TextChannel;
                if (channel) {
          ...
          case "usun": {
            if (channelsList) {
              ...
              let newChannelList = channelsList.replace(channel+",", "");
      ...
      case "ogłoś":
        if (msg.member!.id == "355043764861140993") {
          ...
          let serversRow = await database.Announcement();
          let servers = serversRow.split(":");
          servers.forEach(element => {
            ...
            if (server) {
              let channel = server.channels.cache.get(channelID) as Discord.TextChannel;
              if (channel) {
                channel.send(`**@here Wiadomość od autora: **${background.formatMessage(reason)}`);
              }
            }
          })
        }
