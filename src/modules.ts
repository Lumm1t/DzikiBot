let modules = {
  setEndingDate(time: string, output: string, allow0: boolean) {
    let result;
    var d = new Date();
    ...
    return new Promise<string>((resolve, reject) => {
      ...
      if (arg.length > 3 && arg.length < 2) {
        result = "Zły czas, pamiętaj że długość może się składać max z 2 liczb i jednej litery.";
        reject(result);
      } else {
        ...
        let args = arg.split("");
          ...
          let syear: string = String(year);
          let smonth: string = String(month);
          let sday: string = String(day);
          let shour: string = String(hour);
          let sminutes: string = String(minutes);
